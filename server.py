...
def nexrad():
    ...
    a = datetime(2020, 1, 2, 23, 55, 59, 342380)
    print("year =", a.year)
    print("month =", a.month)
    ...
    for i,scan in enumerate(results.iter_success(),start=0):
        ...
        #print(paths_loc,i+1)
        image_name=scan.radar_id+"_"+scan.scan_time.strftime("%Y%m%d_%H")
        #if paths_loc in db:
         #   return image
...
#print(image_loc)    
    return render_template('name.html', name = image_name, url =image_loc)
